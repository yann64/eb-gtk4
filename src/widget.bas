...
' The base for every wrapped GObject-derived instance (Widgets, GtkApplication, ...).
TYPE Obj
    handle AS GObj PTR
END TYPE
...
' Sinks a just-created GObject's floating reference (see GInitiallyUnowned)
' and returns it unchanged - every wrapper constructor in this package
' calls this exactly once, right after creating the real widget, so this
' package's own handle always holds a real, counted reference from the
' moment a caller gets it back. Call ObjDestroy exactly once when done
' with it (matches ordinary manual GObject reference-counting discipline -
' this package has no automatic RAII destructor tied to it, since a plain
' TYPE assignment is a memberwise copy of the handle, not a new reference).
FUNCTION SinkHandle(h AS GObj PTR) AS GObj PTR
    CALL g_object_ref_sink(h)
    SinkHandle = h
END FUNCTION

' Drops this package's own reference (see SinkHandle) - call exactly once
' per wrapped value you own, when you're done with it. Safe to call after
' a widget has also been added to a container (the container holds its
...
' language's own `@ProcName` docs for the C-ABI-compatibility rules this
' implies: no STRING parameters/return). `data` is delivered back to the
' handler as its own trailing parameter (eBasic has no closures, so this
' is the only way a handler gets extra context) - pass 0 if unneeded.
